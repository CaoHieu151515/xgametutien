...
const StatusList = ({ effects }: { effects: StatusEffect[] }) => {
    ...
    return (
         <div className="border-t border-slate-700/50 pt-6 mt-6">
            <button
                onClick={() => setIsOpen(!isOpen)}
                className="w-full flex justify-between items-center text-left text-xl font-bold text-amber-300 pb-2"
            >
                <span>Trạng Thái ({effects.length})</span>
                <svg xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 transition-transform duration-300 ${isOpen ? '' : '-rotate-90'}`} viewBox="0 0 20 20" fill="currentColor">
                    <path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" />
                </svg>
            </button>
            {isOpen && (
    ...
    const renderInfoTab = () => (
     ...
     const renderStatsTab = () => (
        <div>
            ...
            <StatusList effects={profile.statusEffects} />
        </div>
    );
    ...
    const renderRelationshipsTab = () => (
        <div className="space-y-3">
             {npcs.length === 0 ? (
                ...
                npcs.map((npc) => {
                    const isDaoLu = npc.isDaoLu;
                    const relationship = isDaoLu ? { text: '❤️ Đạo Lữ', color: 'text-pink-400' } : getRelationshipText(npc.relationship);
                    const relationshipValue = isDaoLu ? 1000 : npc.relationship;
