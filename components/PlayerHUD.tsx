...
import { CharacterProfile, Identity } from '../types';
...
export const PlayerHUD: React.FC<PlayerHUDProps> = ({ profile, activeIdentity }) => {
    const defaultAvatar = getDefaultAvatar(profile.gender);
    const displayName = activeIdentity ? activeIdentity.name : profile.name;
    ...
    return (
        <div className="w-64 flex-shrink-0 p-3 bg-slate-900/70 backdrop-blur-sm border-2 border-amber-500/50 rounded-lg shadow-lg">
            <div className="flex items-center gap-4">
                <img 
                    src={activeIdentity?.imageUrl || profile.avatarUrl || defaultAvatar} 
                    alt="Player Avatar" 
                    className="w-16 h-16 rounded-full object-cover border-2 border-slate-600"
                    onError={(e) => { e.currentTarget.src = defaultAvatar; }}
                />
                <div className="flex-grow min-w-0">
                    <h3 className="text-lg font-bold text-amber-300 truncate">{displayName}</h3>
