...
import { migrateGameState } from '../../utils/migrationService';
...
export const useSaveLogic = (props: UseSaveLogicProps) => {
    ...
    const loadState = useCallback((state: FullGameState) => {
        ...
        log('useSaveLogic.ts', 'Save file migrated to latest version.', 'INFO');
        // --- END MIGRATION STEP ---
        
        setCharacterProfile(migratedState.characterProfile);
        setWorldSettings(migratedState.worldSettings);
        setNpcs(migratedState.npcs);
        setHistory(migratedState.history);
        setChoices(migratedState.choices);
        setGameLog(migratedState.gameLog || []);
        setIdentities(migratedState.identities || []);
        setActiveIdentityId(migratedState.activeIdentityId || null);
